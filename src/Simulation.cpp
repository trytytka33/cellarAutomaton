...
Simulation::Simulation()
    : window(sf::VideoMode({WINDOW_WIDTH, WINDOW_HEIGHT}),
             "Game of Life - Virus Spread", sf::Style::Default),
      grid(SPACE_HEIGHT, std::vector<bool>(SPACE_WIDTH, false)),
      isRunning(false), updateInterval(0.1f), boundryType(PERIODIC),
      rule(CONWAY), nrCykluSymulacji(0), panelHeight(0),
      sidebarWidth(SIDEBAR_WIDTH), windowWidth(WINDOW_WIDTH),
      windowHeight(WINDOW_HEIGHT), map(1280, 792), // tutaj
      obstacles(SPACE_HEIGHT, std::vector<int>(SPACE_WIDTH, 0)),
      conditions(SPACE_HEIGHT * SPACE_WIDTH), states(std::vector<State>()),
      statsText(font), aliveCells(0) {
...
void Simulation::addState(const std::string &name,
                          const std::vector<sf::Vector2f> &points,
                          float population, float avgTemperature,
                          Terrain terrain) {
...
void Simulation::addConditions(const State &state) {
  ...
  if (startX < 0 || startX >= SPACE_WIDTH || startY < 0 ||
      startY >= SPACE_HEIGHT)
    return;

  if (obstacles[startY][startX] != 0 && obstacles[startY][startX] != 3)
    return;
...
void Simulation::initButtons() {
  ...
  buttons.emplace_back(sf::Vector2f(startX, startY),
                       sf::Vector2f(buttonWidth, buttonHeight), "Start/Stop",
                       font, [this]() { isRunning = !isRunning; });

  buttons.emplace_back(sf::Vector2f(startX, startY + (buttonHeight + spacing)),
                       sf::Vector2f(buttonWidth, buttonHeight), "Wyczysc", font,
                       [this]() { clearGrid(); });

  buttons.emplace_back(
      sf::Vector2f(startX, startY + (buttonHeight + spacing) * 2),
      sf::Vector2f(buttonWidth, buttonHeight), "Block 2x2", font,
      [this]() { setBlock(SPACE_WIDTH / 2, SPACE_HEIGHT / 2); });

  buttons.emplace_back(
      sf::Vector2f(startX, startY + (buttonHeight + spacing) * 3),
      sf::Vector2f(buttonWidth, buttonHeight), "Random 15%", font,
      [this]() { setRandom(15); });

  buttons.emplace_back(
      sf::Vector2f(startX, startY + (buttonHeight + spacing) * 4),
      sf::Vector2f(buttonWidth, buttonHeight), "Random 30%", font,
      [this]() { setRandom(30); });

  buttons.emplace_back(
      sf::Vector2f(startX, startY + (buttonHeight + spacing) * 5),
      sf::Vector2f(buttonWidth, buttonHeight), "Szybciej", font,
      [this]() { updateInterval = std::max(0.01f, updateInterval - 0.02f); });

  buttons.emplace_back(
      sf::Vector2f(startX, startY + (buttonHeight + spacing) * 6),
      sf::Vector2f(buttonWidth, buttonHeight), "Wolniej", font,
...
void Simulation::updateWindowDimensions() {
  gridDisplayHeight = windowHeight;
  float cellWidthSize =
      static_cast<float>(windowWidth - sidebarWidth - 50) / SPACE_WIDTH;
  float cellHeightSize =
      static_cast<float>(gridDisplayHeight - 40) / SPACE_HEIGHT;
...
void Simulation::toggleCell(int x, int y) {
  if (x >= 0 && x < SPACE_WIDTH && y >= 0 && y < SPACE_HEIGHT &&
      (obstacles[y][x] == 0 || obstacles[y][x] == 3)) {
    grid[y][x] = !grid[y][x];
  }
}
...
void Simulation::handleInput() {
  while (auto event = window.pollEvent()) {
    ...
    if (const auto *resized = event->getIf<sf::Event::Resized>()) {
      ...
      sf::View view(sf::FloatRect({0, 0}, {static_cast<float>(windowWidth),
    ...
    if (event->getIf<sf::Event::MouseButtonPressed>()) {
      ...
      if (gridY >= 0 && gridY < SPACE_HEIGHT && gridX >= 0 &&
          gridX < SPACE_WIDTH &&
          (obstacles[gridY][gridX] == 0 || obstacles[gridY][gridX] == 3)) {
  ...
  if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
    ...
    int gridX = static_cast<int>((mousePos.x - gridOffsetX) / cellSize);
    int gridY = static_cast<int>((mousePos.y - 20) / cellSize);
    if (gridX >= 0 && gridX < SPACE_WIDTH && gridY >= 0 &&
        gridY < SPACE_HEIGHT &&
        (obstacles[gridY][gridX] == 0 || obstacles[gridY][gridX] == 3)) {
    ...
    for (int x = 1; x < SPACE_WIDTH - 1; x++) {
      ...
      for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
          ...
          float chance =
              (conditions[y * SPACE_WIDTH + x].population * 2.7f +
               conditions[y * SPACE_WIDTH + x].avgTemperature + terrMod) /
              100.0f;
...
std::vector<sf::CircleShape> Simulation::generateShapes(float cellSize) {
...
void Simulation::draw() {
  ...
  sf::RectangleShape sidebar(
      {static_cast<float>(sidebarWidth), static_cast<float>(windowHeight)});
  ...
  for (int y = 0; y < SPACE_HEIGHT; y++) {
    for (int x = 0; x < SPACE_WIDTH; x++) {
      ...
      if (obstacles[y][x] == 1) {
        ...
        cell.setPosition(
            {centerX - cellSize / 2.3f, centerY - cellSize / 2.3f});
